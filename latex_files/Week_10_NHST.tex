...
\begin{enumerate}
\item Prove that in a two-sample, two-tailed t-test, if the observed data $(g^{*}_{1},g^{*}_{2})$ is such that $\mu_{1}(g^{*}_{1})=\mu_{2}(g^{*}_{2})$, then the p-value for the t-test is 1.
...
\begin{enumerate}
    \item $\Sigma$ is a $\sigma$-algebra on $G_{1}\times G_{2}$. (\textit{Hint: Use the fact that $\mathcal{B}$ is a $\sigma$-algebra on $\mathbbm{R}$.})
