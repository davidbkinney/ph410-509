...
One often sees this in applications of mathematics to set theory, where, as we saw above, $\mathbbm{R}^{2}$ is the set of all pairs of real numbers.
...
$$\forall x \exists y \forall z \left[x\in y \iff \forall w (w\in z \Rightarrow w\in x)\right].$$
Note that the formula $\forall w (w\in z \to w\in x)$ just states that $z\subseteq x$ (for all $w$, if $w$ is in $z$, then $w$ is in $x$). So, the power set axiom just says that for all sets $z$, there exists a set $y$ such that, for all sets $z$, $z$ is an element fo $y$ if and only if it is a subset of $x$. This is just the power set. One can prove that every set has a unique power set (and you will do this below). 


\subsection{Union}
This axiom just states that for any two sets, we can define a third set that is the union of the first two. In first-order logic:
$$\forall x \exists y \forall z \left[x\in y \iff \exists w (w\in x \wedge w\in y)\right].$$
Since the union of any two sets is provably unique, we can, as we have done above, write $x\cup y$ for the union of two sets.
...
\subsection{Choice}
This final axiom, which has historically been the most controversial, is the axiom of choice. It states that for any set $x$ is a such that any two non-identical sets $y\in x$ and $y^{\prime}\in x$ have no common elements, there is a set that contains exactly one element of each $y\in x$. In other words, if $x$ is a set of mutually exclusive sets, we can form a new set by \textit{choosing} exactly one element from each set in $x$. In first order logic:
\small
$$\forall x \left[\forall y\forall y^{\prime}(y\in x \wedge y^{\prime} \in x \wedge y\neq\emptyset \wedge y^{\prime} \neq \emptyset \wedge y\neq y^{\prime}
) \Rightarrow y\cap y^{\prime} = \emptyset \right]\Rightarrow \forall w\left[w\in x\Rightarrow(\exists c \exists!z(z\in w \wedge z\in c) \right].$$
\normalsize
...
$+n=\{(a,b)\in\mathbbm{N}^{2}:n+b=a\}.$ For example, the positive integer $+3$ is the set $\{(a,b)\in\mathbbm{N}^{2}:3+b=a\}$. By contrast, the negative integer $-n$ is the set $-i=\{(a,b)\in\mathbbm{N}^{2}:a+n=b\}$ (e.g., $-3$ is $\{(a,b)\in\mathbbm{N}^{2}:a+3=b\}$.\par 
...
where $g$ is the addition function on the ordinals.

\subsection{Rational Numbers}
To mathematics, the \textbf{rational numbers} are ratios of integers. To define the rational numbers as sets, we first define a multiplication function $h$ on pairs of integers as follows:
\begin{equation}
...
Each rational number $q$ can be expressed as a set of pairs of integers:
$$\{(a,b)\in\mathbbm{Z}\times(\mathbbm{Z}\cup\{0\}):b\cdot q=a\}.$$
This is the same thing as saying that a rational number $q$ is the set of all pairs of integers $(a,b)$ such that $\frac{a}{b}=q$. Note that the second term in the pair cannot be zero or else the relation would be undefined.
...
\begin{enumerate}
    ...
    \item Prove that the existence of the Russell set is inconsistent with the axioms of ZFC. (Hint:\ It helps to first prove that there is no set of all sets, using either the power set or regularity axioms).

    \item Prove using von Neumann ordinals that for any finite ordinal $n$, $n=\{0,1,\dots,n-1\}$. 

    \item Prove that $\omega\in(\omega+1)$ but $(\omega + 1)\not\in\omega$. 

    \item Prove that the set of rational numbers $\mathbbm{Q}$ has the cardinality $\aleph_{0}$.
\end{enumerate}
